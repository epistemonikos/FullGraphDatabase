...
def log_insert(self, node, returned, from_systematic_review=None):
    ...
    if returned:
      ...
      info = "NoinsertedNode"
    file.write( "{}\t{}\t{}\t{}\n".format(info, node.klass().encode('utf-8'), node.get_id().encode('utf-8'), node.to_json()) )
    file.close()
...
def log_make_reference(self, node_1, node_2, returned):
    ...
    file.write( "{}\t{}\t{}\t{}\t{}\n".format(info, node_1.get_id().encode('utf-8'), node_2.get_id().encode('utf-8'), node_1.to_json(), node_2.to_json())  )
    file.close()